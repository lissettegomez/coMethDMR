...
vignette: >
  ...
  %\VignetteIndexEntry{"Genome-wide methylation analysis using coMethDMR via parallel computing"}
...
## Compute residuals
This step removes uninteresting technical and biological effects using the `GetResiduals` function, so that the resulting co-methylated clusters are only driven by the biological factors we are interested in. **ODOM: For 676 genic regions in serial, this takes 37 seconds. For these regions over 6 cores, this takes 82 seconds.**
```{r, results='hide'}
# a0 <- Sys.time()
...
Once we know how many cores we have available, we execute the `CoMethAllRegions()` function using each worker in the cluster, to find co-methylated clusters in the genic regions. **ODOM: For 676 genic regions, 20 samples, and 8552 sites over 6 cores, this takes 99 seconds.**
