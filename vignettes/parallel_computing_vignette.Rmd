...
Because the output of `CoMethSingleRegion()` is a nested list of lists of two elements, and we only care about the individual CpG subregions (the `CpGsSubregions` elements), we will extract these elements directly.
...
The first element is the data frame that matches the CpG subregion above (with rows ordered by their location):
