...
## Setup
If you want to apply the `coMethDMR` pipeline to all region types, you need a cluster and a list of all regions.
...
## Whole-Workflow Worker
As before, we first create a worker function. This function wraps around both the co-methlyated clustering function and the linear mixed model function, so we combine the previous steps into one.
```{r composed_worker, eval=FALSE}
...
## Apply the Worker over All Region Types
Now that we have this function, we can apply it to all of the regions. This process may take a few hours, depending on your computer's resources and speed.
```{r call_worker, eval=FALSE}
aAll <- Sys.time()
...
Before we move on, let's save these results.
```{r save_master_results, eval=FALSE}
saveRDS(
  resultsAll_ls,
  file = paste0(projectPath_char, "EWAS_PFC_AllRegions.RDS")
)
```

## Wrangle and Save the Results
This results list is rather complex:
...
```{r inspect_resultsAll, eval=FALSE}
resultsAll_ls$NSHORE[[1]]
```
```
$contiguousRegions
                     Region        CpG   Chr   MAPINFO    r_drop keep keep_contiguous
1 chr10:100991959-100992149 cg02800607 chr10 100991959 0.6587985    1               1
2 chr10:100991959-100992149 cg17868515 chr10 100992068 0.7339801    1               1
3 chr10:100991959-100992149 cg24134845 chr10 100992149 0.5568591    1               1

$CpGsSubregions
$CpGsSubregions$`chr10:100991959-100992149`
[1] "cg02800607" "cg17868515" "cg24134845"


$modelFits_df
                          chrom     start       end nCpGs     Estimate      StdErr       Stat    pValue regionType
chr10:100991959-100992149 chr10 100991959 100992149     3 -0.001244083 0.005479532 -0.2270419 0.8203912     NSHORE
```

We want to extract the `modelFits_df` data frames from each region within each region type. This requires nested `lapply()` calls (or we could use the `purrr` package, which is very nice).
```{r extract_sublists, eval=FALSE}
...
And now we save this data frame.
```{r save_results_table, eval=FALSE}
write.csv(
  lmmResults_df,
  file = paste0(projectPath_char, "EWAS_PFC_lmmOut.csv")
)
```
