...
In order to use this list of locations, we can either pass in the `closeByCGI_ls` object, or call for the region type `"ISLAND"` in the co-methylation function below. If you are interested in analyzing other types of genomic regions, see “Introduction to **coMethDMR**”, Section 2.1 for necessary modifications to this code. 
...
Now we execute the `CoMethAllRegions()` function using each worker in the cluster (this takes about 3 minutes over 8 cores with 16Gb of RAM). We select the first Exon regions.
...
```{r BiocParallel_lmmTest, warning=FALSE, results='hide'}
# a1 <- Sys.time()
