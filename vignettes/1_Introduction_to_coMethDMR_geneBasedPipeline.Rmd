...
author: "Lissette Gomez, Gabriel Odom,  Tiago Chedraoui Silva, Lanyu Zhang, Lily Wang"
date: "October 9, 2019"
output:
  word_document:
    toc: yes
  ...
  html_document:
    ...
    toc_float: no
vignette: |
  %\VignetteIndexEntry{"Introduction to coMethDMR"} %\VignetteEncoding{UTF-8}       %\VignetteEngine{knitr::rmarkdown}
---
...
`coMethDMR` is an R package that identifies genomic regions that are both co-methylated and deferentially
...
If you are running the `coMethDMR` package for the first time, you will  also need to install the `sesameData` package. This will just take a few minutes and it will allow you to download complementary data necessary to run `coMethDMR` package otherwise you won't be able to go forward. 
```{r eval=FALSE}
