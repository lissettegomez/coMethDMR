...
author: "Lissette Gomez, Gabriel Odom, Lanyu Zhang, Tiago Chedraoui Silva, Lily Wang"
date: "August 20, 2019"
output:
...
The corresponding phenotype dataset included variables `stage` (Braak AD stage), `subject.id`, `Mplate` (batch effect), `Sex`, `Sample` and `age.brain` (age of the brain donor). Please note the phenotype file needs to have a variable called "Sample" that links to the methylation dataset. 
...
We are interested in identifying co-methylated genomic regions associated with AD stages (`stage` treated as a linear variable). Here we illustrate analysis of genomic regions mapped to CpG islands on chromosome 22. 
...
Genomic regions on the Illumina arrays can be defined based on their relations to genes or CpG Islands (Figure 1). In fact, in our updated pipeline, to reduce redundancy in the tested genomic regions, we recommend first testing genic and intergenic regions first, then add annotations to each genomic region for their relation to CpG islands. 

In `coMethDMR` package, for 450k arrays, the relevant genomic regions are in files `450k_Gene_3_200.RDS` and `450k_InterGene_3_200.RDS`. For EPIC arrays, the relevant genomic regions are in files `EPIC_Gene_3_200.RDS` and `EPIC_InterGene_3_200.RDS`.

These files were created using the function `WriteCloseByAllRegions`, briefly, for genic regions, within each gene, we identified clusters of CpGs located closely (i.e. the maximum separation between any two consecutive probes is 200bp; `maxGap = 200`), and we required each cluster to have at least 3 CpGs (`minCpGs = 3`). For intergeic regions, we identified clusters CpGs similarly for each chromosome. 

These pre-computed genomic regions can be accessed using the following commands: 
