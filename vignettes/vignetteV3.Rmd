...
We are interested in identifying co-methylated genomic regions associated `disease stage` (treated as a linear variable). Here we illustrate analysis of genomic regions mapped to CpG islands, however the workflow can be similarly conducted for other types of genomic region as well. See section xx below.   
...
Here, CpGisland_ls is a list of 20 items, with each item of the list including a group of CpG probe IDs located closely within a particular CpG island region. Section xx discusses how to import additional genomic regions. 
...
Next, we test these co-methylated regions against `disease stage` using a random coefficient model (more details in section xx below), adjusting for `age.brain`.  
