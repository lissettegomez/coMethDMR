...
There are several steps: (1) obtain genomic regions mapped to CpG islands, (2) identify co-methylated regions, and (3) test co-methylated regions against the outcome variable AD stage. 
...
`coMeth_ls` is list with several components. In particular, the component `coMeth_ls$CpGsSubregions` is a list that contains groups of CpG probeIDs corresponding to co-methylated regions. Three comethylated regions were identified in this example. 
...
The second list `coMeth_ls$contiguousRegions` shows the details on how the co-methylated region was obtained: Here `keep = 1` if `rDropThresh_num > 0.4` (i.e. a co-methylated CpG), and `keep_contigous` indicates if the probe is in a contiguous co-methylated region.   
